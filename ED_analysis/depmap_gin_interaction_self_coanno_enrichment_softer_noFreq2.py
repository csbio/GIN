...
# Description: This file starts at 08232024, in order to run the hypergeometric method pipeline on DepMap 22Q4 data
# and qGI_20211111 to check about gene pair functional enrichment within each standard (GIN/ED/intersection)
...
def co_annotation_self_enrichment_pairwise(num_bg, num_bg_anno, df_subset, df_standard, standard_name='complex', output_direc='./suppressor_analysis_04142023/'):
    ...
    print(pval)
    return [pval, num_sub, num_sub_anno]

def group_coanno_self_test(df_all, df_depmap_neg, df_depmap_pos, df_fc, df_coanno_standard, standard_name, output_dir):
    print(standard_name)
    ...
    # All screened pairs
    [num_bg_all, num_bg_all_anno] = get_annotated_pairs(df_all, df_standard=df_coanno_standard,
                                             gene1_col='library_gene', gene2_col='query_gene',
                                              output_file=None)

    # GIN neg & pos
    df_GI_neg = df_all[df_all['GI_standard']=='negative']
    df_GI_pos = df_all[df_all['GI_standard']=='positive']
    [num_GI_neg, num_GI_neg_anno] = get_annotated_pairs(df_GI_neg, df_standard=df_coanno_standard, 
                                                 gene1_col='library_gene', gene2_col='query_gene', 
                                                  output_file=output_dir+standard_name+'negGI_subset_pairs.csv')
    [num_GI_pos, num_GI_pos_anno] = get_annotated_pairs(df_GI_pos, df_standard=df_coanno_standard, 
                                                 gene1_col='library_gene', gene2_col='query_gene', 
                                                  output_file=output_dir+standard_name+'posGI_subset_pairs.csv')

    GI_neg_pval = hyper_test(x=num_GI_neg_anno, M=num_bg_all, n=num_bg_all_anno, N=num_GI_neg)
    GI_pos_pval = hyper_test(x=num_GI_pos_anno, M=num_bg_all, n=num_bg_all_anno, N=num_GI_pos)

    print("co-anno enrichment on negative GIs p-value:", GI_neg_pval)
    print("co-anno enrichment on positive GIs p-value:", GI_pos_pval)

    # ED
    [num_ED_neg, num_ED_neg_anno] = get_annotated_pairs(df_depmap_neg, df_standard=df_coanno_standard, 
                                                 gene1_col='library_gene', gene2_col='query_gene', 
                                                  output_file=output_dir+standard_name+'negED_subset_pairs.csv')
    [num_ED_pos, num_ED_pos_anno] = get_annotated_pairs(df_depmap_pos, df_standard=df_coanno_standard, 
                                                 gene1_col='library_gene', gene2_col='query_gene', 
                                                  output_file=output_dir+standard_name+'posED_subset_pairs.csv')

    ED_neg_pval = hyper_test(x=num_ED_neg_anno, M=num_bg_all, n=num_bg_all_anno, N=num_ED_neg)
    ED_pos_pval = hyper_test(x=num_ED_pos_anno, M=num_bg_all, n=num_bg_all_anno, N=num_ED_pos)

    print("co-anno enrichment on negative EDs p-value:", ED_neg_pval)
    print("co-anno enrichment on positive EDs p-value:", ED_pos_pval)
    ...
    print("co-anno enrichment on ED & GI intersection p-value:")
    print('\nnegPCC posGI')
    [pval_np, num_sub_np, num_sub_np_anno] = co_annotation_self_enrichment_pairwise(num_bg=num_bg_all, num_bg_anno=num_bg_all_anno, df_subset=df_depmap_neg_qGI_pos_fc,
                                      df_standard=df_coanno_standard, standard_name=standard_name,
                                      output_direc=output_dir+'negPCC_posGI_')
    print('\nnegPCC negGI')
    [pval_nn, num_sub_nn, num_sub_nn_anno] = co_annotation_self_enrichment_pairwise(num_bg=num_bg_all, num_bg_anno=num_bg_all_anno, df_subset=df_depmap_neg_qGI_neg_fc,
                                      df_standard=df_coanno_standard, standard_name=standard_name,
                                      output_direc=output_dir+'negPCC_negGI_')
    print('\nposPCC posGI')
    [pval_pp, num_sub_pp, num_sub_pp_anno] = co_annotation_self_enrichment_pairwise(num_bg=num_bg_all, num_bg_anno=num_bg_all_anno, df_subset=df_depmap_pos_qGI_pos_fc,
                                      df_standard=df_coanno_standard, standard_name=standard_name,
                                      output_direc=output_dir+'posPCC_posGI_')
    print('\nposPCC negGI')
    [pval_pn, num_sub_pn, num_sub_pn_anno] = co_annotation_self_enrichment_pairwise(num_bg=num_bg_all, num_bg_anno=num_bg_all_anno, df_subset=df_depmap_pos_qGI_neg_fc,
                                      df_standard=df_coanno_standard, standard_name=standard_name,
                                      output_direc=output_dir+'posPCC_negGI_')
    print("\n\n\n")
    bg_ratio = float(num_bg_all_anno / num_bg_all)
    dict_summary  = {
        'Background': num_bg_all,
        'Background_anno': num_bg_all_anno,
        'GI_neg': num_GI_neg,
        'GI_neg_anno': num_GI_neg_anno, 
        'GI_neg_fold': float(num_GI_neg_anno / num_GI_neg) / bg_ratio, 
        'GI_neg_pval': GI_neg_pval, 
        'GI_pos': num_GI_pos,
        'GI_pos_anno': num_GI_pos_anno, 
        'GI_pos_fold': float(num_GI_pos_anno / num_GI_pos) / bg_ratio, 
        'GI_pos_pval': GI_pos_pval, 
        'ED_neg': num_ED_neg,
        'ED_neg_anno': num_ED_neg_anno, 
        'ED_neg_fold': float(num_ED_neg_anno / num_ED_neg) / bg_ratio, 
        'ED_neg_pval': ED_neg_pval, 
        'ED_pos': num_ED_pos,
        'ED_pos_anno': num_ED_pos_anno, 
        'ED_pos_fold': float(num_ED_pos_anno / num_ED_pos) / bg_ratio, 
        'ED_pos_pval': ED_pos_pval, 
        'Overlap_negED_posGI': num_sub_np,
        'Overlap_negED_posGI_anno': num_sub_np_anno,
        'Overlap_negED_posGI_fold': float(num_sub_np_anno / num_sub_np) / bg_ratio, 
        'Overlap_negED_posGI_pval': pval_np,
        'Overlap_negED_negGI': num_sub_nn,
        'Overlap_negED_negGI_anno': num_sub_nn_anno,
        'Overlap_negED_negGI_fold': float(num_sub_nn_anno / num_sub_nn) / bg_ratio, 
        'Overlap_negED_negGI_pval': pval_nn,
        'Overlap_posED_posGI': num_sub_pp,
        'Overlap_posED_posGI_anno': num_sub_pp_anno,
        'Overlap_posED_posGI_fold': float(num_sub_pp_anno / num_sub_pp) / bg_ratio, 
        'Overlap_posED_posGI_pval': pval_pp,
        'Overlap_posED_negGI': num_sub_pn,
        'Overlap_posED_negGI_anno': num_sub_pn_anno,
        'Overlap_posED_negGI_fold': float(num_sub_pn_anno / num_sub_pn) / bg_ratio, 
        'Overlap_posED_negGI_pval': pval_pn,
    }
    df_summary = pd.DataFrame.from_dict(dict_summary, orient='index').T
    print(df_summary)
    df_summary.to_csv(output_dir+standard_name+'self_enrich_summary.csv', index=False)
...
if __name__ == "__main__":
    ...
    # paralog enrichment
    df_paralog_standard_co = pd.read_csv('./dataset/coannotation_paralog_pairs_new_2024.txt', sep='\t', index_col='gene1')
    ...
    group_coanno_self_test(df_depmap_neg_abba_soft, df_depmap_pos_abba_soft, df_fc, 
    ...
    output_dir_softer = "./output_softer_skipStep12_noFreq2_self/"
    # Include paralogs
    group_coanno_self_test(df_qGI_nRed_depmap, df_depmap_neg_abba_soft, df_depmap_pos_abba_soft, df_fc, 
            df_coanno_standard=df_paralog_standard_co, standard_name="paralog", output_dir=output_dir_softer)
    group_coanno_self_test(df_qGI_nRed_depmap, df_depmap_neg_abba_soft, df_depmap_pos_abba_soft, df_fc, 
            df_coanno_standard=df_corum_standard_co, standard_name="CORUM", output_dir=output_dir_softer)
    group_coanno_self_test(df_qGI_nRed_depmap, df_depmap_neg_abba_soft, df_depmap_pos_abba_soft, df_fc, 
    ...
    # Exclude paralogs:
    print("\n\n\n\n\nExclude paralogs:")
    df_qGI_nRed_depmap = exlude_paralogs(df_qGI_nRed_depmap, df_paralogs)
    ...
    output_dir_softer_np = "./output_softer_skipStep12_noFreq2_self_noPara/"
