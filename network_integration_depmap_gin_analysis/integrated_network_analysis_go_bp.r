...
# load GO-BP standard provided with FLEX
load(file='./FLEX_R-master/data/data_GO_BP.rda')
# generate co-annotation data from GO-BP standard
...
#generate GO-BP standard mapping to generate PR curve for depmap data
go <- CalculatePredictionAndTrueOnLibraryProfiles(go_bp, sim_net)
corum_sim <- list(list(true = go$true, predicted = go$predicted))

# generate AUPRC data against GO-BP standard for depmap data and save to file
generate_auprc_data(go, data_GO_BP, go_bp, output_folder, 'depmap20q2_go')

# remove mitochondrial gene pairs from the GO-BP standard mapping to generate PR curve for depmap data
go <- getSubsetOfCoAnnRemovePairs(go_bp, data.frame(go), list(mito_data_genes$V1), replace = F)
corum_sim_no_mito <- list(list(true = go$true, predicted = go$predicted))
...
# generate GOBP standard mapping to generate PR curve for bionic integrated features
complex <- CalculatePredictionAndTrueOnLibraryProfiles(go_bp, sim_net) #create mapping for PR curves
corum_sim <- append(corum_sim, list(list(true = complex$true, predicted = complex$predicted)))

# generate AUPRC data against GOBP standard for bionic integrated features and save to file
generate_auprc_data(complex, data_GO_BP, go_bp, output_folder, 'Gin-DepMap-params4_features_go_bp')

# remove mitochondrial gene pairs from the GOBP standard mapping to generate PR curve for bionic integrated features
complex <- getSubsetOfCoAnnRemovePairs(go_bp, data.frame(complex), list(mito_data_genes$V1), replace = F)
...
# generate GOBP standard mapping to generate PR curve for GI data
complex <- CalculatePredictionAndTrueOnLibraryProfiles(go_bp, sim_net)
corum_sim <- append(corum_sim, list(list(true = complex$true, predicted = complex$predicted)))

# generate AUPRC data against GOBP standard for GI data and save to file
generate_auprc_data(complex, data_GO_BP, go_bp, output_folder, 'qGI_20211111_fullFF_go_bp')

# remove mitochondrial gene pairs from the GOBP standard mapping to generate PR curve for GI data
complex <- getSubsetOfCoAnnRemovePairs(go_bp, data.frame(complex), list(mito_data_genes$V1), replace = F)
corum_sim_no_mito <- append(corum_sim_no_mito,list(list(true = complex$true, predicted = complex$predicted)))

# generate PR-curve plots

file_ext <- "depmap_gin_bionic-rpco_all gene pairs_GO_BP_"
generate_corum_similarity_plot(corum_sim, output_folder, file_ext, curve_labels, col_set, "" )

file_ext <- "depmap_gin_bionic-rpco_no mito. gene pairs_GO_BP_"
generate_corum_similarity_plot(corum_sim_no_mito, output_folder, paste(file_ext,'no_mito',sep=''), curve_labels, col_set, "" )
...
depmap_file <- 'AUPRC_depmap20q2_go.txt'
...
gin_file <- 'AUPRC_qGI_20211111_fullFF_go_bp.txt'
...
int_file <- 'AUPRC_Gin-DepMap-params4_features_go_bp.txt'
...
{
  cmplx_gene_list <- data_GO_BP[data_GO_BP$ID == cmplx,]$Genes
...
write.csv(auprc_all,"./output/AUPRC_merged_depmap_gin_bionic_rpco_go_bp.csv",quote = T)

#############################################
# generate scatter plot -- bionic vs depmap # 
#############################################

#auprc_all = read.csv("./output/AUPRC_merged_depmap_gin_bionic_rpco_go_bp.csv")
#output_folder <- './output/'
data = auprc_all
...
# selected = c('protein deneddylation','preassembly of GPI anchor in ER membrane', 
#              'GPI anchor biosynthetic process',
#              'folic acid metabolic process',
#              'signal transduction involved in G2 DNA damage checkpoint'
# )#go bp
selected = c('338','16254',
             '6506',
             '46655',
             '72425'
)
...
ggsave(file.path(output_folder, "scatter_auprc_depmap_bionic_go_bp.pdf"),height = 10 , width = 10 )
...
ggsave(file.path(output_folder, "scatter_auprc_gin_bionic_go_bp.pdf"),height = 10 , width = 10 )
...
ggsave(file.path(output_folder, "scatter_auprc_depmap_gin_go_bp.pdf"),height = 10 , width = 10 )
