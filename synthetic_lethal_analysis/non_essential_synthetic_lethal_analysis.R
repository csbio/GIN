...
all_significant_neg_GIs = all_significant_neg_GIs[which(all_significant_neg_GIs$library_gene %in% NE_genes),]
...
get_screen_type<-function(x)
{
  x1<-strsplit(x,"_")[[1]][3] #split x with delimiter _ and retrieve first sub-string
  return( x1)
}
...
write.csv(all_significant_neg_GIs,"./output/SL_information.csv",row.names=F)
...
for(index in c(1,2,3)){
	...
	t_list = seq(-3, as.numeric(limit_list[index]), by = step_list[index])
	print(t_list)
	gene_count = c()
	pair_count = c()
	for(t in t_list){
	...
	output_folder = "./output/"
	ggsave(file.path(output_folder, paste('NE_SL_3_',abs(limit_list[index]),'.pdf',sep='')), arrangeGrob(plot1, plot2),height = 15 , width = 10)
}