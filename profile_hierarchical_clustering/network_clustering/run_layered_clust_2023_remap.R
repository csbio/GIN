...
setwd("/project/GIN/layered_clust/")
source(file.path("../src", "clusterv2.R"))
scores <- read.csv("/project/GIN/input/qGI_centroid12_rm_no507Var2.txt", sep = "\t", row.names = 1)
output_folder <- file.path("output_4k_2023_remap", "clusters_v2_expanded", "qGI_exp_redundant_c12rm_noVar2_queries_snr")
if (!dir.exists(output_folder)) { dir.create(output_folder, recursive = TRUE) }

# Sets parameters that determine the number of clusters for each run
n_layers <- 3
metric_thresholds <- c(0.01, 0.01, 0.01)
...
# Filters by the genes in the expanded network
expanded_genes <- read.csv(file.path("input/expanded_network_3784nodes_01112023.txt"), stringsAsFactors = FALSE, sep = "\t")
scores <- scores[rownames(scores) %in% expanded_genes$name,]
