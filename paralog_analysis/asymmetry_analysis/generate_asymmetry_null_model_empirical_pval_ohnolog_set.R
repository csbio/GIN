...
library("qvalue", lib.loc="../../local/R/library/")
...
ohnolog_sub_both_exp = ohnolog[which(ohnolog$Symbol1 %in% supp_table_sub$`Gene Symbol` & ohnolog$Symbol2 %in% supp_table_sub$`Gene Symbol`),]

# get degree information of non-redundant library genes
degree_data = read.csv('./qGI_degree_auc_ess.txt',sep = '\t')

ohnolog_sub_both_exp = ohnolog_sub_both_exp[which(ohnolog_sub_both_exp$Symbol1 %in% degree_data$gene & ohnolog_sub_both_exp$Symbol2 %in% degree_data$gene),]

ohnolog_sub_both_exp_ = ohnolog_sub_both_exp[,c('Symbol1','Symbol2','sorted_pair')]

# get negative interaction degree of each gene pair 
ohnolog_sub_both_exp_$qGI_degree_std_gene1 = degree_data$qGI_degree_std_neg[match(ohnolog_sub_both_exp_$Symbol1,degree_data$gene)]
ohnolog_sub_both_exp_$qGI_degree_std_gene2 = degree_data$qGI_degree_std_neg[match(ohnolog_sub_both_exp_$Symbol2,degree_data$gene)]
...
write.csv(average_null_delta,"./average_null_negative_GI_ratio_ohnolog_hsapiens.Pairs.Relaxed.2R.csv",row.names=F)

print(average_real_delta)
# https://rdrr.io/bioc/qvalue/man/empPvals.html
# https://rdrr.io/bioc/qvalue/src/R/empPvals.R 
null_delta = read.csv(average_null_delta,"./average_null_negative_GI_ratio_ohnolog_hsapiens.Pairs.Relaxed.2R.csv")
...
emp_p <- 1-unlist(lapply(c(real_delta), p_fun))
print(emp_p) 