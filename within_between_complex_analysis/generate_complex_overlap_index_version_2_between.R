...
#load between complex analysis output file: WB_library_3_or_query_1_complex_subset_5_between_.csv
data_qGI <- read.csv('./output_library_3_or_query_1_complex/WB_library_3_or_query_1_complex_subset_5_between.csv',
                     header = TRUE, stringsAsFactors = FALSE)

# for each complex (both in a pair) in the between complex analysis output file, get the complex size in terms of # of member genes
# required to get a list of complexes ordered by size
comlex_list <- unique(union(unique(data_qGI$Row_Complex),unique(data_qGI$Column_Complex)))
...
#save to file
write.csv(sim_jacrd,'./output_library_3_or_query_1_complex/subset_5_between_overlap_index_version_2.csv',row.names = F)
