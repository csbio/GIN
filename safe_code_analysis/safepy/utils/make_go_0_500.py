...
def add_description_to_go_matrix(go_matrix, df_go_details):
    ...
    for go_term in go_term_list:
        new_go_list.append(go_term + ' ' + df_go_details.loc[go_term]['name'])
...
def make_go_matrix(path_to_obo='', path_to_annotations='', go_branch='p'):
    ...
    txt_path = os.path.join(dirname_annotations, 'go_' + go_branch + '_matrix_0_500.txt')
    print("Printing the gene-to-term matrix at %s" % txt_path)
    go_matrix.to_csv(txt_path, sep='\t')

    # Add GO term names
    go_matrix_desc = add_description_to_go_matrix(go_matrix, go['go_details'])
    txt_path_desc = os.path.join(dirname_annotations, 'go_' + go_branch + '_matrix_0_500_with_description.txt')
    go_matrix_desc.to_csv(txt_path_desc, sep='\t')
...
def make_locus2term(go_graph, go_annotations):
    ...
    locus2term_df_full = pd.DataFrame(data=locus2term, index=loci, columns=go_terms) # df name changed by Xiang
    print(locus2term_df_full.shape)
    ...
    # Added by Xiang Zhang on 12/03/2021 for a filter of (0, 500)
    num_loci_x_term = locus2term_df_full.sum(axis=0)
    num_loci_x_term_0 = num_loci_x_term[num_loci_x_term > 0]
    num_loci_x_term_0_500 = num_loci_x_term_0[num_loci_x_term_0 < 500]
    locus2term_df = locus2term_df_full.loc[:, num_loci_x_term_0_500.index]
    print(locus2term_df.shape)
...
if __name__ == '__main__':
    ...
    '''
    # Add GO term names
    go_matrix = pd.read_csv('/Users/zhangxiang/Downloads/Research_CB/GIN/safepy_network_annotation/safe_go_matrix_output/GO_BP_0_500/go_p_matrix_0_500.txt',index_col=0,sep='\t')

    path_to_obo = args.path_to_obo
    go = get_go_graph(path_to_obo)
    
    go_matrix_desc = add_description_to_go_matrix(go_matrix, go['go_details'])
    txt_path_desc = '/Users/zhangxiang/Downloads/Research_CB/GIN/safepy_network_annotation/safe_go_matrix_output/GO_BP_0_500/go_p_matrix_0_500_with_name.txt'
    go_matrix_desc.to_csv(txt_path_desc, sep='\t')
    '''
