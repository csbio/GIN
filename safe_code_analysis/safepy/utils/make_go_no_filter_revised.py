...
def make_go_matrix(path_to_obo='', path_to_annotations='', go_branch='p'):
    ...
    txt_path = os.path.join(dirname_annotations, 'go_' + go_branch + '_matrix_no_filter.txt')
    ...
    txt_path_desc = os.path.join(dirname_annotations, 'go_' + go_branch + '_matrix_no_filter_with_description.txt')
...
'''
# Old - not full propagation - only remember one path
...
def store_predecessors_all(graph, node_id=None, predecessors=[]):
    ...
    for n in other_nodes:
        ...
        graph.node[n]['predecessors'] = unique_predecessors
...
def make_locus2term(go_graph, go_annotations):
    ...
    for go_term in tqdm(go_terms):
        ...
        # Xiang 04302023: Change from 1 to 2 for gene name mapping
    ...
    locus2term_df[roots[0]] = locus2term_df[roots[0]]
